import React from "react";
import { Guru } from "@public/assets/icons/Guru";
import { LaptopCoffeeBulb } from "@public/assets/icons/LaptopCoffeeBulb";

export const Meditation: React.FC = () => {
  return (
    <div className="flex_center relative">
      <object
        data="data:image/svg+xml;base64,"
        type="image/svg+xml"
      />
      <div className="absolute right-1/2 -translate-x-1/2 animate-[wiggle_5s_ease-in-out_infinite]">
        <Guru />
      </div>
      <div className="absolute right-1/2 -translate-x-1/2 animate-[wiggle_4s_ease-in-out_infinite]">
        <LaptopCoffeeBulb />
      </div>
    </div>
  );
};
