...
import { BsGithub, BsLinkedin, BsFiletypePdf, BsTelegram } from 'react-icons/bs'
...
const SocialLinks: React.FC = () => {
	const onClick = () => {
		fetch('_Игорь-Степанов-Frontend-разработчик.pdf').then(response => {
			response.blob().then(blob => {
				const fileURL = window.URL.createObjectURL(blob)
				let alink = document.createElement('a')
				alink.href = fileURL
				alink.download = '_Игорь-Степанов-Frontend-разработчик.pdf'
				alink.click()
			})
		})
	}
